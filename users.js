...
exports.login = function(host_id,password,call){
	async.auto({
		...
		check_passwpord:['check_if_user_exist','get_passwpord',function(callback,results){
			bcrypt.compare(password, results.get_passwpord, function(err, res) {
				// res == true
				if(res == true){
					//设置登陆状态为online
					var timestamp = Date.now();
					client.zadd('users:online',timestamp,results.check_if_user_exist);
		...
		//用户密码加密
			bcrypt.genSalt(10, function(err, salt) {
				bcrypt.hash(password, salt, function(err, hash) {
					client.hmset('user:'+results.get_next_uid,'username',host_id,'password',hash,'picture',host_id_pic,'email',email,function(err,status){
						console.log(err);
						callback(err,status);
					});
				});
